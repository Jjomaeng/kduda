...
def target_strong_transform(param, target_img=None):
    assert (target_img is not None) #(3,512,512)
    target_img,target= one_mix_target(mask=param['mix_target'], data=target_img,target = target_img)
    target_img,_ = color_jitter(
        color_jitter=param['color_jitter'],
        s=param['color_jitter_s'],
        p=param['color_jitter_p'],
        mean=param['mean'],
        ...
        data=target_img,
        target=None)
    target_img, _ = gaussian_blur(blur=param['blur'], data=target_img, target=None)
    return target_img,target
...
def one_mix_target(mask, data=None,target = None):
    if mask is None:
        return data, target
    if not (data is None):
        stackedMask0, _ = torch.broadcast_tensors(mask[0], data[0])
